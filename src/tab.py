...
def render_tab(bars,
    ...
    # all bars so each rendered tab-note gets a unique data-i. Callers that
    # render several staves into one card (e.g. the full-song page's stacked
    # guitars) pass note_seq_start so indices stay globally unique and line up
    # with the single concatenated audio sequence.
    note_seq = note_seq_start

    # fixed_px pins the SVG to an exact pixel size via an inline style (which
    # beats the .card svg { width:100% } rule). The full-song page uses it so
    # every stacked stave keeps the same pixels-per-bar — a note is the same
    # size on every line, and a short stave isn't blown up to fill the card.
    # Without it the width attribute is just a natural size the card scales.
    px = (f' style="width:{width}px;height:{total_height}px;max-width:none"'
          if fixed_px else '')
    out = [f'<svg class="tab tab-rich" width="{width}"{px} viewBox="0 0 {width} {total_height}" xmlns="http://www.w3.org/2000/svg">']
    if title:
    ...
    for line_idx, line_bars in enumerate(lines):
        ...
        for bi, bar in enumerate(line_bars):
            ...
            for ev in bar:
                ...
                # Inject the sequential note index into note/chord events so the
                # playhead can light up each tab-note in order. The counter must
                # advance on EVERY event — rests included — because the audio
                # sequence (audio_from_bars + playSequence) indexes rests too, so
                # skipping them here would mis-light every note after the first
                # rest (the "etude 3" bug).
            ...
            # Palm-mute bracket spanning consecutive pm events. Covers chords
            # too — the song's palm-muted gallops are power-chord stabs.
            pm_runs = []
            i = 0
            def _is_pm(info):
                return info['ev'][0] in ('note', 'chord') and info['ev'][-1].get('pm')
            while i < len(event_info_list):
                if _is_pm(event_info_list[i]):
                    j = i
                    while j < len(event_info_list) and _is_pm(event_info_list[j]):
                        j += 1
